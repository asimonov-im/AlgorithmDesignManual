namespace AlgorithmDesign.Exercises.Chapter8
{
    ...
    public class EditDistanceWithSwap
    {
        ...
        {
            var helper = new EditDistanceWithSwap(text, pattern);
            return helper.GetEdits();
        }

        public EditDistanceWithSwap(string text, string pattern)
        {
            this.text = text;
                ...
                for (int j = 1; j <= pattern.Length; ++j)
                {
                    if (text[i - 1] == pattern[j - 1])
                    {
                        costs[0] = (EditType.Match, DP[i - 1, j - 1]);
                             ...
                             text[i - 2] == pattern[j - 1])
                    {
                        costs[0] = (EditType.Swap, DP[i - 2, j - 2] + 1);
                    }
                    else
                    {
